...
if __name__ == '__main__':
    ...
    extra_idx = np.arange(len(train_data))
    ...
    for data_name, data in zip(['test', 'train'], [train_data, test_data]):
        x0_pool = np.hstack([sol['x'][:, :1] for sol in data])
        t_eval = np.arange(0., config.t_int + config.dt_save / 2.,
                           config.dt_save)

        for controller in controllers:
            sims[data_name][str(controller)], _ = monte_carlo(
                ocp, controller, x0_pool, [0., config.t_int], t_eval=t_eval,
                **config.sim_kwargs)

            # Save simulation data
            save_data(sims[data_name][str(controller)],
