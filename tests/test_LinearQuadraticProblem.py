...
def test_missing_inits(missing):
    ...
    with pytest.raises(RuntimeError, match=missing):
        ocp = LinearQuadraticProblem(**init_dict)
...
def test_cost_functions(n_states, n_controls, n_samples):
    ...
    # saturated.
    fin_diff_dLdu2 = (
        super(LinearQuadraticProblem, ocp)
    ...
    if n_samples == 1:
        ...
        dLdx, dLdu = ocp.running_cost_gradients(x.flatten(), u.flatten())
        assert dLdx.shape == (n_states,)
        assert dLdu.shape == (n_controls,)

        dLdx, dLdu = ocp.running_cost_hessians(x.flatten(), u.flatten())
...
def test_dynamics(n_states, n_controls, n_samples):
    ...
    # saturated.
    fin_diff_dfdu = (
        super(LinearQuadraticProblem, ocp).jacobians(x, u, return_dfdx=False)
    )
    ...
    if n_samples == 1:
        ...
        dfdx, dfdu = ocp.jacobians(x.flatten(), u.flatten())
...
def test_optimal_control(n_states, n_controls, n_samples):
    ...
    dudx = ocp.optimal_control_jacobian(x, p)
    assert dudx.shape == (n_controls, n_states, n_samples)

    compare_finite_difference(
        x, dudx, lambda x: ocp.optimal_control(x, p),
    ...
    if n_samples == 1:
        ...
        dudx = ocp.optimal_control_jacobian(x.flatten(), p.flatten())
